...
class PrepWork(BaseRecord):
    def __init__(self, prepID, downloadMLX, excelInspectUploaded, inspectionPictures, postedSync, formComplete,
                 concretePatchNeeded, materialOrderNumberHD,
                 cpn_eta, inspectionDueDates, lastDateChecked, fkCreatedUser, createdTimeStamp, fkLastModifiedUser,
                 lastModifiedTimeStamp, fkWorkOrderName):
        self.prepID = prepID
    ...
    def set_fk_work_order_names(self, __fk_work_order_names):
        if not __fk_work_order_names:
            raise ValueError(
                "Work order name must have value. Work Order Name: {wod}".format(wod=__fk_work_order_names))
